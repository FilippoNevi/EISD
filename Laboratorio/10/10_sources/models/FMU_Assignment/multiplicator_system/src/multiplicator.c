...
#define max(a,b) ((a)>(b) ? (a) : (b))
    ...
    fmi2Status fmi2GetInteger(fmi2Component c, const fmi2ValueReference vr[],size_t nvr, fmi2Integer value[]) {
        ...
        for (i = 0; i < nvr; i++) {
            ...
            if (vrOutOfRange(comp, "fmi2GetInteger", vr[i], NUMBER_OF_INTEGERS))return fmi2Error;
            value[i] = integer(vr[i])
    ...
    fmi2Status fmi2SetInteger(fmi2Component c, const fmi2ValueReference vr[],size_t nvr, const fmi2Integer value[]) {
        ...
        FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetInteger: nvr = %d", nvr)
            ...
            for (j = 0; j < nvr; j++) {
                ...
                if (vrOutOfRange(comp, "fmi2SetInteger", vr[j], NUMBER_OF_INTEGERS))return fmi2Error;

                FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetInteger: #i%d# = %d",vr[j], value[j])
                    integer(vr[j]) = value[j];
    ...
    fmi2Status fmi2SetBoolean(fmi2Component c, const fmi2ValueReference vr[],size_t nvr, const fmi2Boolean value[]) {
        ...
        FILTERED_LOG(comp, fmi2OK, LOG_FMI_CALL, "fmi2SetBoolean: nvr = %d", nvr)
            ...
            for (i = 0; i < nvr; i++) {
                if (vrOutOfRange(comp, "fmi2SetBoolean", vr[i], NUMBER_OF_BOOLEANS))return fmi2Error;
